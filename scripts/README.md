...
- `import:wordpress:data` parses the MySQL dump, writes
  `src/_data/wordpressPosts.json` and the complete
  `src/_data/wordpressTaxonomies.json` archive index, rewrites obsolete
  internal URLs, and writes the referenced-media manifest.
...
- `import:wordpress:validate` tests the SQL and URL parsers, verifies exactly
  194 unique legacy permalinks and every category/tag from the dump, rejects
